...
const TeacherVerification: React.FC = () => {
  ...
  const fetchTeachers = async () => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8001/api/admin/auth/teachers/?status=${selectedStatus}`);
  ...
  const fetchTeacherDocuments = async (teacherId: string) => {
    try {
      const response = await fetch(`http://localhost:8001/api/admin/auth/teachers/${teacherId}/documents/`);
      const data = await response.json();
  ...
  const updateTeacherStatus = async (teacherId: string, status: string) => {
    try {
      const response = await fetch(`http://localhost:8001/api/admin/auth/teachers/${teacherId}/status/`, {
        method: 'POST',
