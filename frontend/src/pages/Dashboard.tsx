...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setStudentSession(session as StudentSession);
    fetchDashboardStats(session.id);
    fetchRecentCourses(session.id);
    fetchRecentActivity(session.id);
    loadUserTheme(session.id);
  }, [navigate]);

  const loadUserTheme = async (studentId: number) => {
    try {
      const response = await fetch(`http://localhost:8080/api/auth/get_student?student_id=${studentId}`);
  ...
  const fetchDashboardStats = async (studentId: number) => {
    try {
      const response = await fetch(`http://localhost:8080/api/courses/dashboard_stats?student_id=${studentId}`);
      const data = await response.json();
  ...
  const fetchRecentCourses = async (studentId: number) => {
    try {
      const response = await fetch(`http://localhost:8080/api/courses/my_courses?student_id=${studentId}`);
      const data = await response.json();
      
      if (data.status === 'success') {
        // Get the 2 most recent courses
  ...
  // Format today's date for the header (e.g., Mon, 05 May 2025)
  ...
  const ongoingCourses: Course[] = [
    {
      ...
      instructor: "Prof. Richard Miller",
      progress: 75,
      nextLesson: "Integration Techniques",
      thumbnail: "https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=500",
    ...
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1501504905252-473c47e087f8?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=500",
  ...
  const recommendedCourses: Course[] = [
    {
      ...
      rating: 4.9,
      level: "Intermediate",
      thumbnail: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=500",
    ...
    },
    {
      ...
      rating: 4.8,
      level: "Intermediate",
      thumbnail: "https://images.unsplash.com/photo-1555774698-0b77e0d5fac6?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=500",
  ...
  const fetchRecentActivity = async (studentId: number) => {
    try {
      console.log('Fetching recent activity for student:', studentId);
      const response = await fetch(`http://localhost:8080/api/courses/recent_activity?student_id=${studentId}`);
      const data = await response.json();
        ...
        <div className="dashboard-content">
            ...
            <div className="progress-dashboard">
              <div className="section-header">
                ...
                <Link to="/performance" className="view-all-btn">
                  Detailed Analytics
                  <FaArrowRight />
                </Link>
              </div>
              ...
              <div className="progress-metrics-row">
                <div className="progress-metric">
                  ...
                  <div className="metric-info">
                    <h4>Overall Score</h4>
                    <p className="metric-trend positive">+8% this week</p>
                  </div>
                </div>
                ...
                <div className="progress-metric">
                  <div className="metric-circle orange">
                    <svg viewBox="0 0 36 36" className="circular-chart">
                      <path className="circle-bg" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                      <path className="circle orange" strokeDasharray="75, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
              ...
              <div className="performance-summary">
                <div className="summary-card-large">
                  <div className="summary-header">
                    ...
                    <div className="summary-content">
                      <h3>Top 15% in Class</h3>
                      <p>You're performing excellently across all subjects!</p>
                  ...
                  <Link to="/performance" className="summary-cta">
                ...
                <div className="insights-grid">
                  <div className="insight-item">
                    ...
                    <p>Completed 3 new lessons and improved overall score by 8%</p>
                  </div>
                  
                  <div className="insight-item">
                    <div className="insight-icon">⭐</div>
                    <h4>New Milestone</h4>
                    <p>Reached Level 3 and unlocked advanced learning features</p>
                  </div>
                  
                  <div className="insight-item">
                    <div className="insight-icon">🎯</div>
                    <h4>Learning Momentum</h4>
                    <p>On track to complete 2 more courses this month</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="recommended-grid">
                {recommendedCourses.map(course => (
                  <div key={course.id} className="recommended-card">
                    <div className="recommended-thumbnail" style={{ backgroundImage: `url(${course.thumbnail})` }}>
                    ...
                    <div className="recommended-content">
                      ...
                      <span className="recommended-category">{course.category}</span>
            ...
            {/* Skill Badges & Endorsements Section */}
            <div className="dashboard-section">
              <div className="badges-endorsements-grid">
                <SkillBadges studentId={studentSession?.id || 0} compact={true} />
                <EndorsementWidget studentId={studentSession?.id || 0} />
              </div>
            </div>
