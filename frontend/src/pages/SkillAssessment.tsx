...
const SkillAssessment = () => {
  ...
  const [startedAssessments, setStartedAssessments] = useState<{[key: number]: boolean}>({});
  ...
  const fetchEnrolledCourses = async () => {
    try {
      ...
      if (data.status === 'success') {
        ...
        // Fetch assessment results for each course
        const coursesWithResults = await Promise.all(
          data.data.map(async (course: any) => {
            try {
              ...
              console.log(`Course ${course.id} assessment result:`, assessmentResult);
  ...
  const generateQuestions = (courseTitle: string, category: string): Question[] => {
    ...
    return questionBank[category] || questionBank['Programming'];
  };
  ...
  const submitAssessment = async () => {
    ...
    try {
      ...
      if (session) {
        ...
        // Store in completedAssessments state for immediate UI update
        const newResult = {
          id: Date.now(),
          course_id: selectedAssessment.courseId,
          score: finalScore,
...
Congratulations!
  ...
  const resetAssessment = async () => {
    ...
    setShowResult(false);
    setScore(0);
    // Immediately refresh courses to show updated results
      ...
      <StudentLayout>
        <div style={{ padding: '20px', paddingTop: '100px', maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ background: 'white', padding: '40px', borderRadius: '12px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
            ...
            <h1 style={{ fontSize: '2rem', marginBottom: '10px' }}>Assessment Complete!</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '20px' }}>
            {enrolledCourses.map(course => (
              <div key={course.id} style={{ 
                ...
                {completedAssessments[course.id] || course.assessment_result || startedAssessments[course.id] ? (
                  <div>
                    {completedAssessments[course.id] || course.assessment_result ? (
                      ...
                      <div style={{ textAlign: 'center', padding: '20px', background: '#f3f4f6', borderRadius: '8px' }}>
                        <p style={{ color: '#666', marginBottom: '15px' }}>Assessment in progress...</p>
                        <button 
                          onClick={() => startAssessment(course)}
                          style={{
                            padding: '12px 24px',
